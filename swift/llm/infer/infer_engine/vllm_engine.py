...
class VllmEngine(InferEngine):
    ...
    def _add_request(self,
        ...
        print(f'[vLLM Engine DEBUG] inputs keys: {list(inputs.keys())}', flush=True)
        if 'ecg_features' in inputs:
            ecg_f = inputs['ecg_features']
            print(f'[vLLM Engine DEBUG] ecg_features found! type={type(ecg_f)}, shape={getattr(ecg_f, "shape", "N/A")}', flush=True)
        else:
            print(f'[vLLM Engine DEBUG] ecg_features NOT in inputs!', flush=True)
        ...
        if self._version_ge('0.4.3'):
            ...
            # Add ECG to multi_modal_data (via monkey patching).
            ecg_features = inputs.get('ecg_features')
            if ecg_features is not None:
                mm_data['ecg'] = ecg_features
                print(f'[vLLM Engine DEBUG] ECG features added to mm_data, shape={ecg_features.shape}', flush=True)
            
            if mm_data:
                llm_inputs['multi_modal_data'] = mm_data
                print(f'[vLLM Engine DEBUG] mm_data keys: {list(mm_data.keys())}', flush=True)
