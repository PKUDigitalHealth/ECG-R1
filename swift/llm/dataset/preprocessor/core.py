...
class RowPreprocessor:
    ...
    def __init__(self,
        ...
        self._error_stats = {}  # Track error types
    ...
    def _check_objects(row):
        ...
        new_objects = {}
        # Ensure the order
        for k in ['ecg','ref', 'bbox', 'bbox_type', 'image_id']:
            if k in objects.keys():
                new_objects[k] = objects[k]
        
        # Only check bbox if it exists
        if 'bbox' not in new_objects:
            return
    ...
    def batched_preprocess(self, batched_row: Dict[str, Any], *, strict: bool,
        ...
        for row in rows:
            try:
                ...
                error_key = f"{error_type}: {str(e)[:100]}"
        ...
        *,
        ...
        if isinstance(dataset_mapped, HfDataset) and len(dataset) != len(dataset_mapped):
            ...
            if self._error_stats:
                logger.info('='*80)
                logger.info('Dataset Error Statistics:')
                logger.info('='*80)
                total_errors = sum(self._error_stats.values())
                sorted_errors = sorted(self._error_stats.items(), key=lambda x: x[1], reverse=True)
                for error_msg, count in sorted_errors[:20]:  # Top 20 errors
                    percentage = count / total_errors * 100
                    logger.info(f'  [{count:>6} occurrences, {percentage:>5.1f}%] {error_msg}')
                if len(sorted_errors) > 20:
                    logger.info(f'  ... and {len(sorted_errors) - 20} more error types')
                logger.info('='*80)
